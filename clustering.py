...
from djset import *
from contours import TemporalContour


@njit(nogil=True)
def cluster_hybrid_deltaE(cielab_video, mask, delta, labels, label_set, neighbor_offsets):
    ...
    for f, r, c in mask:
        ...
        indices = candidate_neighbors[..., 0] * pixels_per_frame + candidate_neighbors[..., 1] * width + candidate_neighbors[..., 2]
...
def cluster(cielab_video: np.ndarray, mask: np.ndarray, neighbor_offsets, threshold: float = 10.0):
    ...
    cluster_hybrid_deltaE(cielab_video, foreground_pixels, threshold, labels, label_set, neighbor_offsets)
    ...
    return np.array([TemporalContour(np.array(contour)) for contour in components.values()])
